...
Fixpoint r2l'' f (x: designSpace) (xBest: designSpace) (yBest : range) : designSpace :=
  let y := (ev x f) in
  let (xBest', yBest') := if y <? yBest then (x, y) else (xBest, yBest) in
  match x with 
    | 0   => xBest 
...
Lemma defaultArgumentsWork: forall f, (r2l f) = (r2l' f 10).
Proof.
  intros.
  auto.
Qed.  

Definition r2l' (f:objFun) (x__init : designSpace) := 
